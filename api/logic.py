...
def get_degree_of_sim(v1, v2):
    ...
    dot1 = ((dot(v1, v2)/len([x for x in v1 if x == 1])) +
            (dot(v1, v2)/len([y for y in v2 if y == 1])*2)) / 3
    dot2 = ((dot(v1, v2)/len([x for x in v1 if x == 1]))
            * 2 + (dot(v1, v2)/len([y for y in v2 if y == 1]))) / 3
...
def inter(animes, choice, check, amnt):
    ...
    musts_check = [217, 17, 15, 35, 53, 68, 190,
                   143, 106, 114, 116, 160, 19, 54, 6, 144]
    must = [x for x in range(len(choice["vector"]))
            if x in musts_check and choice["vector"][x] == 1]
    ...
    for x in animes:
        ...
        if 1 in anime_vec:
            degree_of_similarity = get_degree_of_sim(
                choice_anime_vector, anime_vec)
...
def ranking(recomendation_list, amount_to_recommend, choice):
    ...
    # check to see if the amount proposed is less than the amount asked
    if len(recomendation_list) < amount_to_recommend:
        return []
    ...
    for anime in recomendation_list:  # different attributes
        ...
        if "" not in check and " " not in check and None not in check and stop_doubles_in_ranking(anime_rec_names, name):
            anime["key"] = (1+(anime["simi"])) + (float(check[0])/100 * math.log10(check[1]+0.00000001)/4) + (1/check[3] * 1/2) + \
                0.3/(1 + abs(int(choice["attributes"]["startDate"].split("-")
                     [0]) - int(check[5].split("-")[0])))  # applying formula

            anime_rec_inter.append(anime)
            anime_rec_names.append(name)

    anime_rec_ranking = sorted(
        anime_rec_inter, key=lambda k: k["key"], reverse=True)
...
def write_input(anime, number):
    ...
    with open('./data/inputs.csv', 'a', newline='') as f:
